...
class ColossusView(View):

    methods = ["GET"]

    def __init__(self):
        View.__init__(self)
        self.mysqique = Mystique.build(os.environ)

    def _test_cookie(self):

        if self.mysqique.COOKIE not in flask.request.cookies:
            ...
            raise BadRequestException(
                self.mysqique.get_bastion_redirect_url(nxt))
    ...
    def _issue_request_to_data_server(self):
        ...
        return self.mysqique.get_data_response(
            flask.request.path,
            flask.request.args,
            flask.request.cookies[self.mysqique.COOKIE]
        )
    ...
    def dispatch_request(self, *args, **kwargs):
        ...
        # it as a new cookie.
        response.set_cookie(
            self.mysqique.COOKIE,
            self.mysqique.generate_bastion_cookie(data_response.headers)
        )
